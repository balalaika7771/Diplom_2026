...
Архитектура включает шесть модулей, организованных в конвейер обработки инцидентов. Общая схема представлена на рисунке~\ref{fig:system_architecture}; детализированная архитектура и поток данных --- на рисунках~\ref{fig:appendix_infrastructure} и~\ref{fig:appendix_agent_flow} в Приложении~А.

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=0.8cm, auto, scale=0.95, transform shape, font=\small]
...
\centering
\small
\caption{Модули системы и их функции}
...
\hline
Collector & Сбор метрик, логов, трейсов & Prometheus, Elasticsearch, Jaeger \\
\hline
Analyzer & Обнаружение аномалий & Isolation Forest, LSTM \\
\hline
Diagnostic & Построение причинного графа & Корреляция, Granger \\
\hline
LLM Agent & Генерация рекомендаций & GPT-4, MCP \\
\hline
Orchestrator & Консенсус и планирование & Взвешенное голосование \\
\hline
Executor & Безопасное исполнение & Kubernetes API \\
\hline
\end{tabular}
\end{table}
...
\begin{tikzpicture}[scale=0.7, transform shape, font=\small,
    agent/.style={rectangle, draw, fill=blue!15, minimum width=1.5cm, minimum height=0.6cm}]
    \node[agent] (de) {Diagnostic};
    \node[agent, below=0.5cm of de] (llm) {LLM};
    \node[agent, below=0.5cm of llm] (rb) {Rule-based};
    \node[agent, right=2cm of llm] (orch) {Orchestrator};
    \node[agent, right=2cm of orch] (exec) {Executor};
    \draw[->] (de) -- (orch);
    \draw[->] (llm) -- (orch);
    \draw[->] (rb) -- (orch);
    \draw[->] (orch) -- (exec);
\end{tikzpicture}
\caption{Схема консенсуса агентов}
\label{fig:agent_consensus}
\end{figure}
...
\centering
\small
\caption{Уровни безопасности действий восстановления}
\label{tab:safety_levels}
\begin{tabular}{|>{\raggedright\arraybackslash}p{2cm}|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{2.5cm}|}
\hline
Уровень & Условие & Режим & Примеры \\
\hline
Low & Risk $< \theta_1$ & Автоматический & Перезапуск пода \\
\hline
Medium & $\theta_1 \leq$ Risk $< \theta_2$ & Gradual rollout & Масштабирование \\
\hline
High & $\theta_2 \leq$ Risk $< \theta_3$ & С подтверждением & Откат деплоя \\
\hline
Critical & Risk $\geq \theta_3$ & Только вручную & Изменение конфигурации \\
\hline
\end{tabular}
\end{table}
...
\centering
\begin{tikzpicture}[scale=0.7, transform shape, font=\small,
    box/.style={rectangle, draw, fill=orange!15, minimum width=1.5cm, minimum height=0.6cm}]
    \node[box] (validate) {Валидация};
    \node[box, right=1cm of validate] (gradual) {Gradual};
    \node[box, right=1cm of gradual] (rollback) {Откат};
    \node[box, right=1cm of rollback] (limit) {Лимит};
    \draw[->] (validate) -- (gradual);
    \draw[->] (gradual) -- (rollback);
    \draw[->] (rollback) -- (limit);
\end{tikzpicture}
\caption{Этапы безопасного исполнения}
\label{fig:safe_execution}
\end{figure}
