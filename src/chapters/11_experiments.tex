...
\textbf{Разделение выборок.} Данные разделены по времени (а не случайно) для предотвращения data leakage: первые 60\% по времени --- обучение, следующие 20\% --- валидация (подбор порога $\theta$), последние 20\% --- тест. Все результаты в таблицах~\ref{tab:scenario1_results}--\ref{tab:scenario3_results} получены на тестовой выборке.
