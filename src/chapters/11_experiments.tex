...
Система демонстрирует эффект обучения --- точность обнаружения и диагностики растёт с накоплением опыта.
...
\hline
Внутренняя & Субъективность оценки LLM-компоненты & Экспертная группа из 3 инженеров, шкала от 1 до 5; согласие экспертов (Cohen's $\kappa$) = 0.74 \\
\hline
Внешняя & Привязка к конкретному стеку (Kubernetes, Prometheus, Kafka); перенос требует калибровки & Использование реальных данных (30\%) и типовых паттернов отказов \cite{netflix_chaos,aiops_survey} наряду с синтетическими сценариями (70\%) \\
\hline
Конструктная & 4 сценария покрывают основные классы инцидентов, но не включают многокластерные аварии и проблемы безопасности & Расширение набора сценариев --- направление дальнейших исследований \\
\hline
\end{tabular}
\end{table}
...
\begin{axis}[
    ...
    xtick=data,
    x tick label style={rotate=15, anchor=east, font=\normalsize},
    nodes near coords,
