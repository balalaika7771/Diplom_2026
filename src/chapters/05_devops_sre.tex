...
\begin{tikzpicture}[scale=0.7, transform shape, font=\small,
    stage/.style={rectangle, draw, fill=green!15, minimum width=1.2cm, minimum height=0.6cm}]
    \node[stage] (build) {Build};
    \node[stage, right=0.8cm of build] (test) {Test};
    \node[stage, right=0.8cm of test] (deploy) {Deploy};
    \node[stage, right=0.8cm of deploy] (verify) {Verify};
    \draw[->] (build) -- (test);
    \draw[->] (test) -- (deploy);
    \draw[->] (deploy) -- (verify);
\end{tikzpicture}
\caption{Этапы CI/CD pipeline}
...
Kubernetes обеспечивает базовый уровень самовосстановления \cite{burns_kubernetes,kubernetes_self_healing,kubernetes_docs,container_orchestration}, на который надстраиваются агенты.
...
\centering
\small
\caption{Сравнение стратегий развёртывания}
\label{tab:deploy_strategies}
\begin{tabular}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{4.5cm}|}
\hline
Стратегия & Риск & Характеристика \\
\hline
Blue-Green & Низкий & Две среды, мгновенный откат \\
\hline
Canary & Средний & Постепенное увеличение трафика \\
\hline
Rolling & Средний & Пошаговое обновление реплик \\
\hline
\end{tabular}
\end{table}
...
\centering
\small
\caption{Примеры SLO и соответствующих SLI}
\label{tab:slo_sli_examples}
\begin{tabular}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{4cm}|}
\hline
Сервис & SLO & SLI \\
\hline
API Gateway & Доступность 99.9\% & Uptime / Total time \\
\hline
Auth Service & Latency $<$ 100 мс & p99 response time \\
\hline
Database & Ошибки $<$ 0.1\% & Failed queries / Total \\
\hline
\end{tabular}
\end{table}
