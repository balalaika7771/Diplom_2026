...
\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=0.8cm, auto, scale=1.0, transform shape, font=\normalsize]
...
\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=1.5cm, auto, scale=1.0, transform shape, font=\normalsize]
...
\begin{itemize}
    ...
    \item \textbf{Зависимость от LLM-провайдера.} Качество рекомендаций LLM Agent зависит от выбранной модели и её доступности. При недоступности LLM-провайдера (сетевой сбой, rate limit) система продолжает работать без LLM-обогащения, но с потерей 4.6\% $F_1$ (ablation study, раздел~\ref{ch:experiments}).
