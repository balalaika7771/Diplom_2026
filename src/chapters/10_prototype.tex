...
Прототип реализует архитектуру системы интеллектуальных агентов (раздел~\ref{ch:architecture}) в виде работающего программного комплекса. Цель прототипа --- проверить гипотезу (\ref{eq:hypothesis_ineq}): обеспечить уменьшение среднего времени восстановления $\text{MTTR}$ и улучшение метрик $m \in \{\text{MTTR}, \text{precision}, \text{false\_positive\_rate}, \text{diagnosis\_time}\}$ на измеримые величины $I_m$, определённые во введении.
...
\end{axis}
\end{tikzpicture}
\caption{Динамика загрузки CPU при отказе и восстановлении сервиса (сценарий~1)}
