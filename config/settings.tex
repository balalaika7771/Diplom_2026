...
% Убираем вертикальные отступы в списках (itemize, enumerate)
\makeatletter
\let\orig@listi\@listi
\let\orig@listii\@listii
\let\orig@listiii\@listiii
\def\@listi{\orig@listi\topsep=0pt\itemsep=0pt\parsep=0pt\partopsep=0pt}
\def\@listii{\orig@listii\topsep=0pt\itemsep=0pt\parsep=0pt\partopsep=0pt}
\def\@listiii{\orig@listiii\topsep=0pt\itemsep=0pt\parsep=0pt\partopsep=0pt}
\makeatother

% Дополнительная растяжимость и толерантность для предотвращения Overfull hbox
\tolerance=1000
...
% Для разделов используем только номер без слова "РАЗДЕЛ" или "ГЛАВА"
...
% Настройки библиографии по ГОСТу (biblatex)
...
% URL formatting handled by biblatex-gost automatically
...
% Настройка шрифта URL (monospace для избежания пробелов в math mode)
\urlstyle{tt}

% Настройки нумерации по ГОСТу 7.32-2017
% Глобальная нумерация рисунков и таблиц по всей диссертации
\makeatletter
...
% Настройка шрифта для verbatim (используем доступный моноширинный шрифт)
% Переопределяем шрифт verbatim для использования ttfamily вместо Courier
\makeatletter
\def\verbatim@font{\ttfamily}
\makeatother

% Окружение для форматированного verbatim с рамкой
\usepackage{fancyvrb}
\DefineVerbatimEnvironment{codeverbatim}{Verbatim}{frame=single, framesep=2mm, numbers=left, numbersep=3pt}
...
\lstset{
    ...
    showtabs=false,
    tabsize=2,
    basicstyle=\small\fontfamily{cmtt}\selectfont, % ~12pt (GOST min), more compact
    columns=flexible,
...
% \captionsetup[table]{format=gosttable,skip=6pt} % уже задано выше
...
% Нумерация рисунков и таблиц в приложениях по ГОСТ 7.32-2017 (п. 6.5.5, 6.6.4)
% Рисунок А.1, Таблица А.1 и т.д.
\makeatletter
\AtBeginDocument{%
...
% babel уже загружен в packages.tex
...
% \autoref и \nameref не переопределяются — используются стандартные из hyperref

% Автоматические правила для абзацев
\AtBeginDocument{\setlength{\parskip}{0pt}}  % без отступов между абзацами (после всех пакетов)
...
\biburlbigskip=0mu\relax  % Убираем растяжимые пробелы в URL библиографии
...
\newcommand{\autoblockdiagram}[3]{%
    \begin{figure}[H]%
        ...
        \begin{tikzpicture}[node distance=1.8cm, scale=0.8, transform shape]%
            ...
            \draw [arrow] (decision) -- node[anchor=south] {Нет} ++(-2.5,0) |- (process);%
...
\newcommand{\autotimingdiagram}[3]{%
    \begin{figure}[H]%
        ...
        \begin{tikzpicture}[scale=0.6, transform shape]%
            \draw[thick] (0,0) -- (10,0) node[right] {Время};%
            \draw[thick] (0,0) -- (0,4) node[above] {Сигналы};%
            %
            \draw[blue,thick] (0,3) -- (2,3) -- (2,1) -- (4,1) -- (4,3) -- (6,3) -- (6,1) -- (8,1) -- (8,3) -- (10,3);%
            \node[blue,left] at (0,3) {Сигнал 1};%
            %
            \draw[red,thick] (0,2) -- (1,2) -- (1,0.5) -- (3,0.5) -- (3,2) -- (5,2) -- (5,0.5) -- (7,0.5) -- (7,2) -- (10,2);%
            \node[red,left] at (0,2) {Сигнал 2};%
            %
            \draw[green,thick] (0,1) -- (1.5,1) -- (1.5,0.2) -- (3.5,0.2) -- (3.5,1) -- (5.5,1) -- (5.5,0.2) -- (7.5,0.2) -- (7.5,1) -- (10,1);%
...
\newcommand{\autostructure}[3]{%
    \begin{figure}[H]%
        \centering%
        \begin{tikzpicture}[node distance=2cm, scale=0.7, transform shape]%
            \node (input) [rectangle, draw=black, fill=blue!20, minimum width=2cm, minimum height=1cm] {Входной сигнал};%
            \node (block1) [rectangle, draw=black, fill=green!20, minimum width=2cm, minimum height=1cm, right=of input] {Блок 1};%
            \node (block2) [rectangle, draw=black, fill=orange!20, minimum width=2cm, minimum height=1cm, right=of block1] {Блок 2};%
            \node (output) [rectangle, draw=black, fill=red!20, minimum width=2cm, minimum height=1cm, right=of block2] {Выходной сигнал};%
            %
...
\newcommand{\autofunctional}[3]{%
    \begin{figure}[H]%
        ...
        \begin{tikzpicture}[node distance=1.8cm, scale=0.7, transform shape]%
            \node (input) [circle, draw=black, fill=blue!20, minimum size=1.5cm] {Вход};%
            \node (process1) [rectangle, draw=black, fill=green!20, minimum width=2cm, minimum height=1cm, right=of input] {Обработка 1};%
            \node (process2) [rectangle, draw=black, fill=orange!20, minimum width=2cm, minimum height=1cm, below=of process1] {Обработка 2};%
            \node (output) [circle, draw=black, fill=red!20, minimum size=1.5cm, right=of process2] {Выход};%
            %
